...
namespace ManagementSolution.Infrastructure.Repositories.Services
{
    public class UserAccount(IOptions<JwtSettings> config, AppDbContext appDbContext) : IUserAccount
    {
        ...
        {
            var user = await appDbContext.AppUsers.FirstOrDefaultAsync(_ => _.Id == id);
        ...
        private async Task<List<ApplicationUser>> GetApplicationUsers() => await appDbContext.AppUsers.AsNoTracking().ToListAsync();

        public async Task<string> GetUserImage(string id) => (await GetApplicationUsers()).FirstOrDefault(_ => _.Id == id)!.Image;

        public async Task<bool> UpdateProfile(UserProfile profile)
        {
            var user = await appDbContext.AppUsers.FirstOrDefaultAsync(_ => _.Id == profile.Id);
